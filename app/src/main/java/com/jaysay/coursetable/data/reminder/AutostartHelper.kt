 ...
 *
 * 上课提醒在应用未打开时依赖系统闹钟触发；厂商 ROM（小米/华为/OPPO/vivo 等）
 * 会把“划掉后台”当作冻结应用并拦截闹钟，实测需要开启“后台活动”才能正常触发。
 * 各厂商启动管理页的入口组件并不稳定，这里依次尝试厂商专用页面，
 * 失败时回退到通用的应用详情设置页（用户可在其中手动找到“后台活动/自启动”开关）。
 */
