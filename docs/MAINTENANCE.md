...
- 当前版本：3.4.36，versionCode 156
- 构建栈：Gradle 9.1.0 / AGP 8.13.0 / Kotlin 2.2.0（内置 Compose 编译器）/ Compose BOM 2025.06.01
- 应用仅在用户手动检查更新时访问 GitHub Releases，或在用户主动检测 AI 连接／确认课表转换后访问所选 HTTPS 接口；不包含常驻联网客户端，课程、备份、提醒和日历均在本机处理。AI API Key 默认仅在页面内存中；只有用户明确选择保存时才以 Android Keystore 加密存放于非备份私有目录，不写入偏好、备份或日志。
