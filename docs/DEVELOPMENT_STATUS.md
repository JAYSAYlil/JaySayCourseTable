...
## 本轮修复（v2.14.1）：提醒在锁屏/划掉后台后仍不触发

- 调度改用 setAlarmClock 用户可见闹钟：系统把提醒登记为“下一个闹钟”（状态栏闹钟图标），Doze、应用待机、省电模式与多数厂商后台清理下保持准点；不需要任何精确闹钟权限，精确/近似闹钟保留为降级链。
- 设置页提醒区新增“发送测试通知”一键验证 + 最近 3 条触发诊断记录（记录时间与结果/原因）。
- 新增电池优化检查与一键“允许后台运行”引导。
- 模拟器真实时间实测：闹钟准点触发、通知弹出、诊断写入正确；测试通知链路测试通过。

## 版本与安全基线

- 当前开发分支：upgrade/v2.12.4（继续沿用，未新建分支）
- 当前候选版：2.14.1 / versionCode 94
